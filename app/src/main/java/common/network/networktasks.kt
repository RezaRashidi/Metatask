...
class networktasks(val db: TodoDatabaseQueries, val scope: CoroutineScope) {
    val baseurl = "https://45.149.77.49:8080"
    ...
    fun senddiliy() {
        val ydailis = db.getAlldailiess().executeAsList().filter {
            Instant.fromEpochMilliseconds(it.DateAdd).toLocalDateTime(TimeZone.currentSystemDefault()).date == nowdate.minus(DatePeriod(0, 0, 1))
        }.map {
